...
from clases import Configuracion, Estacionamiento, MARCAS, COLORES, TIPOS, TIPOS_PAGO, ARCHIVO_BD, ARCHIVO_CONFIG
...
def calcularMonto(horaEntrada, horaSalida, cfg):
    """
    Funcionalidad:
    Calcula el monto que debe pagar un vehiculo segun el tiempo que permanecio estacionado y la configuracion actual del parqueo (tiempo de gracia y monto por hora).

    Entrada:
    horaEntrada: fecha y hora de entrada, formato "%Y-%m-%d %H:%M:%S".
    horaSalida: fecha y hora de salida, formato "%Y-%m-%d %H:%M:%S".
    cfg: objeto de tipo Configuracion con el monto por hora y el tiempo de gracia.

    Salida:
    Retorna el monto total a pagar. Retorna 0 si el tiempo esta dentro del tiempo de gracia.
    """
    formato = "%Y-%m-%d %H:%M:%S"

    entrada = datetime.strptime(str(horaEntrada), formato)
    salida = datetime.strptime(str(horaSalida), formato)

    diferencia = salida - entrada
    minutos = int(diferencia.total_seconds() // 60)

    if minutos <= cfg.tiempoGracia:
        return 0
    ...
    monto = horas * cfg.montoHora

    return monto



def obtenerVehiculosYVouchers(ventanaPadre=None):
    """
    Funcionalidad:
    Obtiene vehiculos desde una API publica y los estaciona automaticamente
    (reserva masiva) en los espacios de tipo General que esten libres,
    respetando el tope maximo permitido (deja siempre un 5% libre para
    clientes nuevos). La hora de entrada de cada vehiculo se asigna de forma
    aleatoria entre las 7:00 am y la hora actual del sistema. Al finalizar,
    genera el voucher en PDF de cada vehiculo estacionado.

    Entrada:
    ventanaPadre: ventana de tkinter sobre la que se muestran los mensajes
    (puede ser None).

    Salida:
    No retorna ningun valor. Actualiza la lista de objetos Estacionamiento,
    la respalda en memoria secundaria (BD) y genera los vouchers en PDF.
    """
    cfg = cargarConfiguracion()
    if not cfg:
        messagebox.showwarning(
            "Sin Configuracion",
            "Debe configurar el parqueo primero.",
            parent=ventanaPadre
        )
        return
    ...
    idsEspacios = generarIdsEspacios(cfg.tamanoParqueo, cfg.tieneElectrico)

    idsGenerales = []
    for eid in idsEspacios:
        if eid.startswith("G-"):
            idsGenerales.append(eid)
    ...
    idsLibres = []
    for eid in idsGenerales:
        if eid not in ocupados:
            idsLibres.append(eid)

    tope = calcularTopeMaximoMasivo(cfg.tamanoParqueo, cfg.tieneElectrico)
    cantidadAEstacionar = tope - len(ocupados)

    if cantidadAEstacionar > len(idsLibres):
        cantidadAEstacionar = len(idsLibres)

    if cantidadAEstacionar < 0:
        cantidadAEstacionar = 0

    if cantidadAEstacionar == 0:
        messagebox.showinfo(
            "Obtener Vehiculos",
            "No hay espacio disponible para la reserva masiva en este momento.",
            parent=ventanaPadre
        )
        return
    ...
    if apertura >= ahora:
        ...
        segundosRango = int((ahora - apertura).total_seconds())

    vehiculosCreados = 0

    for indice in range(cantidadAEstacionar):
        espacioId = idsLibres[indice]

        placa = "API" + str(100 + indice)
        marcaIdx = (indice % len(MARCAS)) + 1
        colorIdx = (indice % len(COLORES)) + 1
        tipoIdx = (indice % len(TIPOS)) + 1

        segundosAleatorios = random.randint(0, segundosRango)
        horaEntradaDt = apertura + timedelta(seconds=segundosAleatorios)
        horaEntrada = horaEntradaDt.strftime("%Y-%m-%d %H:%M:%S")
        ...
        crearVoucherPdf(nuevoObj)

        vehiculosCreados += 1

    guardarBD(lista)

    mensaje = "Vehiculos obtenidos y estacionados: " + str(vehiculosCreados)
    mensaje += "\nVouchers en PDF generados en la carpeta del proyecto."

    if seObtuvoApi == False:
        mensaje += "\n\nAviso: no se pudo contactar la API de marcas, se uso informacion local."

    messagebox.showinfo("Obtener Vehiculos y Vouchers", mensaje, parent=ventanaPadre)
...
def iniciarAplicacion():
    ...
    def abrirE1():
        obtenerVehiculosYVouchers(raiz)
